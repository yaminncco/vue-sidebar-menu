...
export default function useMenu (props, context) {
  ...
  const setMobileItem = ({ item, itemEl }) => {
    ...
    const { top: sidebarTop, left: sidebarLeft, right: sidebarRight } = sidebarMenuRef.value.getBoundingClientRect()
    let parentHeight
    let parentWidth
    let offsetTop = 0
    let width = 0
    ...
    mobileItemRect.maxWidth = parentWidth <= parseInt(props.width) ? width : parseInt(props.width) - parseInt(props.widthCollapsed)
    mobileItemRect.maxHeight = parentHeight - (bottom - offsetTop)
  }
